...
library(arrow)
df2 = read_parquet("C:/Users/gabri/OneDrive/Documentos/Criptos/RoboTraderBinance_1_4b/src/tests/graficos/DADOS_SIMULADOS_1semana_extra_bck.parquet")
...
p <- ggplot(df %>% filter(moeda_estrategia %in% melhores_medianas$moeda_estrategia), 
  ...
  coord_flip()
p %>% print()
ggsave("melhores medianas.jpg", height = 25, width = 20, units = "cm", dpi = 600)
...
p <- ggplot(df %>% filter(moeda_estrategia %in% melhores_Q1$moeda_estrategia), 
  ...
  coord_flip()
p %>% print()
ggsave("melhores q1.jpg", height = 25, width = 20, units = "cm", dpi = 600)
...
ggplot(contagens, aes(x = as.factor(estrategia), y = percentual, fill = classificacao)) +
  ...
  theme(legend.position = "bottom")
ggsave("BARRA CONTAGEM POSITIVAS FILTRADA.jpg", height = 35, width = 15, units = "cm", dpi = 600)
```
...
lista_estrategias = contagens %>% filter(classificacao == 'Positiva' & percentual >= 50)
...
print(p)
ggsave("analise distribuição - boxplot.jpg", plot = p, height = lista_estrategias %>% length() * 10, width = 50, units = "cm", dpi = 600)
